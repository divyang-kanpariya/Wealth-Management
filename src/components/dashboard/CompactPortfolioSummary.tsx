...
export default function CompactPortfolioSummary({ summary }: CompactPortfolioSummaryProps) {
  ...
  const portfolioItems: DataGridItem[] = [
    ...
    {
      label: (summary?.totalGainLoss || 0) >= 0 ? 'Total Gain' : 'Total Loss',
      value: formatCurrency(Math.abs(summary?.totalGainLoss || 0)),
      subValue: formatPercentage(summary?.totalGainLossPercentage || 0),
      color: (summary?.totalGainLoss || 0) >= 0 ? 'success' : 'danger',
      icon: (summary?.totalGainLoss || 0) >= 0 ? (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    },
    {
      ...
      value: (
        <div className="flex items-center space-x-2">
          <span>{formatPercentage(summary?.totalGainLossPercentage || 0)}</span>
          <StatusIndicator 
            status={getReturnStatus(summary?.totalGainLossPercentage || 0)}
            variant="dot"
            size="sm"
          />
        </div>
      ),
      subValue: (summary?.totalGainLoss || 0) >= 0 ? 'Positive return' : 'Negative return',
      color: (summary?.totalGainLossPercentage || 0) >= 0 ? 'success' : 'danger'
    }
  ]
