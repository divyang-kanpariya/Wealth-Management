...
export const updateSipSchema = z.object({
  ...
  frequency: z.nativeEnum(SIPFrequency).optional(),
  startDate: z.coerce.date().optional(),
  endDate: z.coerce.date().optional().nullable(),
  status: z.nativeEnum(SIPStatus).optional(),
  goalId: z.string().optional().nullable(),
  accountId: z.string().min(1).optional(),
  notes: z.string().optional(),
});
...
// SIP validation helper functions
export function validateSip(data: unknown) {
  return sipSchema.parse(data);
}

export function validateUpdateSip(data: unknown) {
  return updateSipSchema.parse(data);
}

export function validateSipTransaction(data: unknown) {
  return sipTransactionSchema.parse(data);
}